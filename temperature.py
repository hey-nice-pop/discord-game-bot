...
async def process_message(message: Message):
    try:
        with FileLock(LOCK_PATH, timeout=5):
            ...
            if new_file_created:
                await message.channel.send('60度に達しました')
...
async def handle_90_degree_threshold(data: dict, message: Message):
    ...
    reset_temperature(data)
    await message.channel.send('60度にリセットされました')
...
def load_json():
    ...
    try:
        ...
        initial_data = {'temperature': 60, 'message_count': 0, 'yesterday_message_count': 10, 'last_date': str(datetime.date.today()), 'last_reward_date': ''}
